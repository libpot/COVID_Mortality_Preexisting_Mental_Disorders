  ...
  mutate(covid_28 = map(.x = data,
         ...
         all_cause_60 = map(.x = data,
                            ~ cox.zph(coxph(Surv(time_to_death_allcause_60, death_allcause_60) ~ exposure + age_group + sex + infection_year + infection_month + DCCI + strata(matching),
                                            data = .x)))) %>%
  mutate(across(ends_with(c("_60", "_28")),
                ~ map_dbl(., function(schoenfeld_residuals) as.data.frame(schoenfeld_residuals$table)$p[1]))) %>%
  select(-data)
...
schoenfeld_residuals_vacc <- data_main_analysis %>% 
  ...
  nest() %>%   
  mutate(covid_28 = map(.x = data,
                        ~ cox.zph(coxph(Surv(time_to_death_covid_28, death_covid_28) ~ exposure + age_group + sex + infection_year + infection_month + DCCI + vaccination + strata(matching),
                                        data = .x))),
         covid_60 = map(.x = data,
                        ~ cox.zph(coxph(Surv(time_to_death_covid_60, death_covid_60) ~ exposure + age_group + sex + infection_year + infection_month + DCCI + vaccination + strata(matching),
                                        data = .x))),
         all_cause_28 = map(.x = data,
                            ~ cox.zph(coxph(Surv(time_to_death_allcause_28, death_allcause_28) ~ exposure + age_group + sex + infection_year + infection_month + DCCI + vaccination + strata(matching),
                                            data = .x))),
         all_cause_60 = map(.x = data,
                            ~ cox.zph(coxph(Surv(time_to_death_allcause_60, death_allcause_60) ~ exposure + age_group + sex + infection_year + infection_month + DCCI + vaccination + strata(matching),
                                            data = .x)))) %>%
  mutate(across(ends_with(c("_60", "_28")),
                ~ map_dbl(., function(schoenfeld_residuals) as.data.frame(schoenfeld_residuals$table)$p[1]))) %>%
  select(-data)
