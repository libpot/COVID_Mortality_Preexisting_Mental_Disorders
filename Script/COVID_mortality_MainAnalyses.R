...
ddd <- data_main_analysis %>% 
  filter(cohort == "any mental disorder",
         time_period == "period 3",
         exposure_definition == "diagnosed and treated")

mod <- survfit(Surv(time_to_death_covid_28, death_covid_28) ~ exposure,
               data = ddd)
mod2 <- coxph(Surv(time_to_death_covid_28, death_covid_28) ~ exposure,
              data = ddd)
